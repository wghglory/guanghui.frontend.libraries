...
function toCreate(obj) {
    var iWidth = 57;
    var iHeight = 42;
    var iRow = Math.floor(obj.offsetHeight / iHeight);
    var iRowLength = Math.floor(obj.offsetWidth / iWidth);
    var iLength = iRow * iRowLength;
    var sHtml = "";
    var iLeft = (obj.offsetWidth - iWidth * iRowLength) / 2;
    var iTop = (obj.offsetHeight - iHeight * iRow) / 2;
    ...
    var aAArr = toArr(aA, iRow, iRowLength);
...
function toArr(collection, iRow, iRowLength) {
    var aObjs = [];
    for (var i = 0; i < iRow; i++) {
        var arr = [];
        for (var j = 0; j < iRowLength; j++) {
            collection[i * iRowLength + j].x = j;
            collection[i * iRowLength + j].y = i;
            collection[i * iRowLength + j].off = true;
            arr.push(collection[i * iRowLength + j]);
        }
        aObjs.push(arr);
    }
    return aObjs;
